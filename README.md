...
│   ├── candidate.yaml                 # Newest release, including RCs
│   └── rc.yaml                        # Legacy RC-only pointer
...
│   └── staging/                        # Production-shaped candidate environment
├── development/                       # VPN-only build hosts; never platform clusters
│   └── hosts.enc.yaml                 # Encrypted SSH endpoint
├── config/
...
- **candidate** — Newest release, whether GA or RC. Staging and release-like development clusters track this.
- **rc** — Legacy RC-only pointer retained for compatibility.

The CLI selector `latest` resolves through `candidate`. An omitted cluster channel
still defaults to `stable`.
...
4. **Channel**: Every tag advances `channels/candidate.yaml`; GA tags also advance
   `channels/stable.yaml`, while RC tags also advance the legacy `channels/rc.yaml`.
   Semantic-version checks prevent any pointer from moving backward.
